...
If the command is unclear or cannot be determined, respond with:
      ...
      final Map<String, dynamic> parameters = commandJson['parameters'] ?? {};
      final String originalCommand = commandJson['original_command'] ?? text;

      String confirmationText;
      if (command != 'unknown' && command != 'error') {
        if (command == 'navigate' && parameters.containsKey('start_location') && parameters.containsKey('end_location')) {
          final String start = parameters['start_location'];
          final String end = parameters['end_location'];
